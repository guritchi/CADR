...
inline void PipelineSceneGraph::init(CadR::StateSet& root, const std::vector<std::bitset<ShaderState::numOptimizeFlags>>& optimizationLevels, vk::PipelineCache pipelineCache, uint32_t maxTextures)  { destroy(); _deleteLibraries=true; _root=&root; _shaderLibrary=new ShaderLibrary(root.renderer().device(), maxTextures); _pipelineLibrary=new PipelineLibrary(*_shaderLibrary, pipelineCache); }
